...
class Formulario extends Base {
    ...
    function cargarInfoFormulario() {
        ...
        $db = $this->db;
        $table_consultar = $_POST["base_datos"].'.'.$_POST["tabla"];
        $sql = "SHOW COLUMNS FROM $table_consultar";
        $rs = $db->select_all($sql);
        $table ="<br/><br/>";
        $table.="<table  style='width:100%; border-collapse: collapse'> ";
        $table.="<tr class='ui-widget-header'>";
        $table.="<th style='width:30px'></th>";
        $table.="<th style='text-align:left'>CAMPO</th>";
        $table.="<th style='text-align:left'>TITULO</th>";
        $table.="<th style='text-align:center'>GRID</th>";
        $table.="<th style='text-align:center'>FILTRO</th>";
        ...
        foreach ($rs as $key => $rw) {
            ...
            if ($titulo[0] != "_") {
                ...
                $table.="<tr>";
                $table.="<td style='text-align:left'> <input type='checkbox' name='incluir[".$rw['Field']."]' value='S' checked='checked' /> </td>";
                $table.="<td style='width:150px'>".$rw['Field']."</td>";
                $table.="<td> <input type='text' name='titulo[{$rw['Field']}]' value='$titulo' style='width:100%'/> </td>";
                $table.="<td style='text-align:center;width:60px'> <input type='text' name='grid[".$rw['Field']."]' value='$ancho' style='width:60px' /> </td>";
                $table.="<td style='text-align:center;width:60px'> <input type='checkbox' name='filtro[".$rw['Field']."]' value='S' $grid /> </td>";
                $table.="</tr>";
            }
        }
    ...
    function verificar() {
        ...
        if (file_exists($ruta)) {
            ...
            $error.= '<div class="alert alert-light-danger">
                    <p><span class="txt-danger" style="float: left; margin-right: .3em;"></span>
                        La ruta <strong>'.$ruta.'</strong> ya existe. 
        ...
        if (floatval($id)>0) {
            ...
            $error.='<div class="alert alert-light-danger">
                    <p><span class="txt-danger" style="float: left; margin-right: .3em;"></span>
                        La url <strong>'.$menu.'</strong> ya existe. 
                        Si se continua se sobreescribiran los datos</p>
            </div>';
    ...
    function generar() {
        ...
        foreach ($rs as $key => $rw) {
            ...
            if ($rw['Field'] == "created_at") {
                $auditoria.="\$_POST['created_at'] = date('Y-m-d H:i:s');\n";
            }
            ...
            if ($fk) { //Llave foranea. Generar un objeto Select    
                $sql = "SHOW COLUMNS FROM $fk[REFERENCED_TABLE_SCHEMA].$fk[REFERENCED_TABLE_NAME] WHERE FIELD <> 'id' ";
                #$rs2 = $db->query($sql);
                #$rw2 = $db->select_row($sql); // Pprimer camppo
                $rw2 = $db->select_all($sql)[0]; // Segundo campo
                ...
                $obj = sprintf('<div class="form-group row">
                                                     <label for="accion" class="col-sm-2 col-form-label">%s</label>
                                                     <div class="col-sm-10">
                                                      <select id="%s" name="%s" title="%s">
                                                        <?php llenar_combo("%s",true); ?>
                                                      </select> 
                                                     </div>
                                                 </div>', $titulo, $campo, $campo, $titulo, $sql);
            } else { //Generar objeto input
                $oculto = ($rw['Extra'] == "auto_increment") ? ' style="display:none"' : '';
                $modificalble = ($rw['Key'] == "PRI") ? ' class="no-modificable"' : '';

                $obj = sprintf('<div class="form-group row"%4$s>
                                                     <label for="accion" class="col-sm-2 col-form-label">%1$s</label>
                                                     <div class="col-sm-10">
                                                        <input type="text" id="%2$s" name="%2$s" title="%1$s" placeholder="%1$s" maxlength="%3$s" value="" %5$s/>
                                                     </div>
                                                 </div>', $titulo, $campo, $longitud, $oculto, $modificalble);
            }
            ...
            $s.= " AND %s = \'$_GET[%1$s]\'";
                    ...
                    switch ($tipo_dato) {
                        ...
                        case "date":
                            ...
                            $obj = ' <div class="form-group row">
                                                     <label for="accion" class="col-sm-2 col-form-label">'.$titulo.'</label>
                                                     <div class="col-sm-10">
                                                       <input type="'.$type.'" id="'.$campo.'" name="desde_'.$campo.'" maxlength="10" style="width:100%" placeholder="Desde"/> - 
                                                       <input type="'.$type.'" id="'.$campo.'" name="hasta_'.$campo.'" maxlength="10" style="width:100%" placeholder="Hasta"/>
                                                     </div>
                                                 </div>';
                            $codigo_filtro .= sprintf('
        ...
        if ($_GET["%1$s"] != "" && $_GET["%1$s"] != "NULL") { 
            ...
            if ($ancho > 0) {
                ...
                $grid.= '<th data-field="'.$campo.'">'.$titulo.'</th>';
            }
            /*  */
        } //While
