...
// Crear una notificación para un usuario (campana / puntos rojos del sistema)
// Cualquier módulo la puede usar: solo recibe a quién, en qué menú y qué dice.
function crear_notificacion($persona_id, $menu, $tipo, $texto, $url_destino)
{
    global $db;

    // 1. Si no hay usuario destino, no se hace nada
    if ((int)$persona_id <= 0) {
        return false;
    }

    // 2. Insertar la fila en la tabla genérica de notificaciones
    $datos = array(
    ...
    return $db->insert('notificaciones', $datos);
}
...
function llenar_combo($sql, $blanco = false, $predeterminado = "") {
    ...
    foreach ($rs as $key => $rw) {
        ...
        echo "<option $sel value='$rw[0]'>$texto</option>";
    }
}
...
function get_llenar_combo($sql, $blanco = false, $predeterminado = "") {
    ...
    foreach ($rs as $key => $rw) {
        ...
        $sel = (trim($rw[0]) == $predeterminado) ? "selected='selected'" : "";
        $texto = (isset($rw[1]) && $rw[1] !== null && $rw[1] !== '') ? $rw[1] : $rw[0];
        $op.="<option $sel value='$rw[0]'>$texto</option>";
    }
    return $op;
}
...
function cifrar_texto($string,$pass='2qz6^JSB')
{
   $method = 'aes256';
...
function decifrar_texto($string,$pass='2qz6^JSB')
{
   $method = 'aes256';
...
function quitarBR_excel($texto){
      $t = str_replace('<br/>'," ",$texto);
      return $t;
}
...
function getDiasHabiles($fechainicio, $fechafin, $diasferiados = array()) {
        ...
        global $db;    
        if (!$diasferiados) {         
           $info = @$db->select_all("SELECT * FROM festivos WHERE fecha >='$fechainicio' and fecha <='$fechafin'");
...
function validar_token($token){
    ...
    global $db;
    $caduca = date('Y-m-d');
    $rw = $db->select_row("SELECT * FROM admin_token WHERE token = '$token' and caduca>='$caduca' ");
    if ($rw) {//No hay errores de validacion
        return true;
...
function validar_token_login($token){
    ...
    global $db;
    $caduca = date('Y-m-d');
    $rw = $db->select_row("SELECT * FROM admin_token WHERE token = '$token' and caduca>='$caduca' and estado=1 ");
    if ($rw) {//No hay errores de validacion
        $update = array();
        $update['estado']=2;
...
function get_num($gestion,$cliente){

    global $db;
    $sql = "SELECT COUNT(*) AS total
    FROM data_gestion
    WHERE id_gestion=$gestion AND id_cliente=$cliente and visible=1";
...
function resizeImgen($originalImagePath , $resizedImagePath , $newWidth, $newHeight,$type="resize")
{
    
    try {
        require_once 'vendor/gumlet/php-image-resize/lib/ImageResize.php';
        $originalImagePath = trim($originalImagePath);
        ...
        $image = new \Gumlet\ImageResize($originalImagePath);
    ...
    switch ($imageType) {
        ...
        default:
            return $originalImagePath;
            $info   = explode(".", $originalImagePath);
            
            if ( strpos(end($info), 'png') ) {
                $originalImage = imagecreatefrompng($originalImagePath);
            }else if ( strpos(end($info), 'bmp') ){
                $originalImage = imagecreatefrombmp($originalImagePath);
            }else if ( strpos(end($info), 'jpeg') ){
                $originalImage = imagecreatefromjpeg($originalImagePath);
            }else if ( strpos(end($info), 'jpg') ){
                $originalImage = imagecreatefromjpeg($originalImagePath);
            }else{
                return $originalImagePath;
            }
...
include_once ('funciones_select.php');
